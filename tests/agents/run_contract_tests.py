...
# Locate shared-protocol.md — agents that reference it inherit its sections.
_shared_dist = REPO_ROOT / "shared-protocol.md"
_shared_installed = REPO_ROOT / ".claude" / "shared-protocol.md"
SHARED_PROTOCOL_PATH = _shared_dist if _shared_dist.exists() else (_shared_installed if _shared_installed.exists() else None)
SHARED_PROTOCOL_BODY: str = SHARED_PROTOCOL_PATH.read_text() if SHARED_PROTOCOL_PATH else ""
...
def _inherits_shared_protocol(agent: AgentFile) -> bool:
    """True if the agent body references shared-protocol.md AND the file exists."""
    return bool(SHARED_PROTOCOL_BODY) and "shared-protocol.md" in agent.body
...
def test_closing_protocol_sections(agent: AgentFile) -> TestResult:
    ...
    searchable = agent.body
    if _inherits_shared_protocol(agent):
        searchable = agent.body + "\n" + SHARED_PROTOCOL_BODY
    missing = [s for s in required_sections if s not in searchable]
    if missing:
        return TestResult(
            agent=agent.basename,
...
def test_nexus_protocol_present(agent: AgentFile) -> TestResult:
    ...
    - Agents inheriting shared-protocol.md get NEXUS docs from there.
    ...
    # mention of NEXUS (minimum 3 references in combined body).
    has_subsection = "NEXUS PROTOCOL" in searchable
    nexus_mentions = searchable.count("NEXUS")
...
def test_team_coordination_discipline(agent: AgentFile) -> TestResult:
    ...
    cover this. Agents inheriting shared-protocol.md get it from there.
    """
    ...
    if "Team Coordination Discipline" not in searchable:
        return TestResult(
            agent=agent.basename,

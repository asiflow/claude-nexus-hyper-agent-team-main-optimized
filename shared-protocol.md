...
All agents in the 31-agent elite engineering team follow this protocol. Agent-specific customizations (YOUR INTERACTIONS, domain-specific NEXUS syscalls, memory focus areas) remain in each agent's own file.
...
4. **FLAG CROSS-DOMAIN FINDINGS** — If you find something outside your domain, flag for handoff
...
You have a persistent, file-based memory system at `${CLAUDE_PROJECT_DIR}/.claude/agent-memory/<your-agent-name>/`. This directory already exists — write to it directly with the Write tool (do not run mkdir or check for its existence).

Save memories as files with frontmatter (name, description, type) and index them in MEMORY.md in the same directory.

Memory types: `user` (user profile), `feedback` (corrections/confirmations), `project` (ongoing work context), `reference` (external resource pointers).

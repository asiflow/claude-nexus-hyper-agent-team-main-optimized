...
- **14 agents on opus** (deep reasoning required): `cto`, `elite-engineer`, `deep-planner`, `deep-qa`, `deep-reviewer`, `go-expert`, `python-expert`, `typescript-expert`, `database-expert`, `api-expert`, `frontend-platform-engineer`, `meta-agent`, `ai-platform-architect`, `test-engineer`
- **17 agents on sonnet** (coordination, ops, structured reasoning): `orchestrator`, `memory-coordinator`, `benchmark-agent`, `cluster-awareness`, `infra-expert`, `observability-expert`, `recruiter`, `challenger`, `evidence-validator`, `beam-architect`, `beam-sre`, `elixir-engineer`, `go-hybrid-engineer`, `erlang-solutions-consultant`, `session-sentinel`, `intuition-oracle`, `talent-scout`
- **Only override** when the user explicitly instructs cost optimization. In that case, pass `model: "sonnet"` explicitly and note the override in your response.
- **Never downgrade language experts, deep-qa, or deep-reviewer to sonnet** without explicit user approval — false negatives from these agents are expensive.

**Shared protocol:** All agents reference `.claude/shared-protocol.md` for team coordination, NEXUS protocol, dispatch modes, closing protocol, and memory system. This file is loaded once per session instead of duplicated in every agent prompt.
